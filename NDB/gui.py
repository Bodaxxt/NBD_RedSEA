...
class MainApplication:
    ...
    def create_installation_confirmation_tab(self):
        ...
        ttk.Label(form_card, text="Confirm Installation on Aircraft", style='CardTitle.TLabel').pack(anchor='w', pady=(0, 20))
        ...
        if current_cycle:
            current_cycle_num = current_cycle['cycle_number'] if isinstance(current_cycle, dict) else current_cycle[1]
            if current_cycle_num in cycle_values:
                self.confirmation_cycle_combo.set(current_cycle_num)
    ...
    def save_update(self):
        try:
            ...
            for aircraft in selected_aircraft:
                update_id = self.db.record_update(cycle_number, engineer_name, file_path, notes, aircraft, update_datetime)
    ...
    def confirm_installation(self):
        ...
        try:
            ...
            # تحديث حالة التثبيت لكل طيارة مختارة
            for aircraft in selected_aircraft:
                cursor = self.db._get_cursor()
                try:
                    # جلب معرّف التحديث الأخير لهذه الطيارة والدورة
                    cursor.execute('''
