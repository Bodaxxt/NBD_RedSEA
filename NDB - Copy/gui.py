    ...
    def save_update(self):
        try:
            ...
            for aircraft in selected_aircraft:
                ...
                try:
                    cursor.execute('SELECT COUNT(*) AS update_count FROM updates WHERE cycle_number = %s AND aircraft_reg = %s', (cycle_number, aircraft))
                    result = cursor.fetchone()
                    if result and result.get('update_count', 0) > 0:
            ...
            import traceback
            error_msg = f"Failed to save update:\n{type(e).__name__}: {str(e)}\n\n{traceback.format_exc()}"
            print(error_msg)
            messagebox.showerror("Error", f"Failed to save update: {type(e).__name__}: {str(e)}")
    def clear_form(self):
        from datetime import datetime
        ...
        try:
            ...
            for aircraft in selected_aircraft:
                ...
                try:
                    ...
                    result = cursor.fetchone()
                    if result:
                        # result قد يكون dict أو tuple حسب نوع row_factory
                        update_id = result['id'] if isinstance(result, dict) else result[0]
                        self.db.mark_update_installed_on_aircraft(update_id, install_time)
                        confirmed_count += 1
                        print(f"✓ Confirmed {aircraft} (Cycle {cycle_number}, ID: {update_id})")
                    else:
                        print(f"⚠ No update found for {aircraft} in cycle {cycle_number}")
                finally:
                    cursor.close()
            
            if confirmed_count > 0:
                messagebox.showinfo("Success", f"✓ Installation confirmed for {confirmed_count} aircraft!")
                self.clear_confirmation_form()
                self.refresh_dashboard_data()
                self.load_history()
            else:
                messagebox.showwarning("Warning", "No updates were confirmed. Please check the cycle data.")
            
        except Exception as e:
            import traceback
            error_msg = f"Failed to confirm installation:\n\n{str(e)}\n\nDetails:\n{traceback.format_exc()}"
